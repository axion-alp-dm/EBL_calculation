...
if os.path.basename(os.getcwd()) == 'scripts':
    os.chdir("..")
direct_name = str('outputs_systematics_13perct'
...
def chi2_measurs_sys(x_model, x_obs, err_obs):
    err_with_sys = np.sqrt((err_obs)**2. + (sys_percentage*x_obs)**2.)
...
combined_likelihood = (
        LeastSquares(
    igl_ebldata['lambda'], igl_ebldata['nuInu'],
    np.sqrt(igl_ebldata['1 sigma']**2.
            + (sys_percentage * igl_ebldata['nuInu'])**2.),
    fit_igl)
        + LeastSquares((
    emiss_data['lambda'], emiss_data['z']), emiss_data['eje'],
    np.sqrt(((emiss_data['eje_n']+ emiss_data['eje_p']) / 2.)**2.
            + (sys_percentage * emiss_data['eje'])**2.),
    fit_emiss)
        + LeastSquares(
    sfr_data[:, 0], sfr_data[:, 3],
    np.sqrt(((sfr_data[:, 4] + sfr_data[:, 5]) / 2.)**2.
            + (sys_percentage * sfr_data[:, 3])**2.),
    sfr)
        + LeastSquares(
    z_data[:, 0], z_data[:, 1],
    np.sqrt(((z_data[:, 2] + z_data[:, 3]) / 2.)**2.
            + (sys_percentage * z_data[:, 1])**2.),
    metall)
                       )
...
outputs.write(
    'emissivities data <10: ' + str(chi2_measurs_sys(
        fit_emiss((data_emiss_small['lambda'], data_emiss_small['z']), aaa),
...
outputs.write(
    'sfr data: ' + str(chi2_measurs_sys(
        sfr(sfr_data[:, 0], aaa),
        sfr_data[:, 3], (sfr_data[:, 4] + sfr_data[:, 5]) / 2.))
    + '\n')
outputs.write(
    'metallicity data: ' + str(chi2_measurs_sys(
        metall(z_data[:, 0], aaa),
