...
if os.path.basename(os.getcwd()) == 'scripts':
    os.chdir("..")
direct_name = str('test_likelihoods2'
...
for nkey, key in enumerate(config_data['ssp_models']):
    ...
    config_data['ssp_models'][key]['best_params'] = m.values
    config_data['ssp_models'][key]['cov_matrix'] = m.covariance

    print(config_data['ssp_models'])


yaml.dump(config_data, 'outputs/' + direct_name + '/output_data',
...
for nkey, key in enumerate(config_data['ssp_models']):
    ...
    # FIGURE: cob fit
    axes_ebl.plot(xx_plot,
                  10 ** ebl_class.ebl_ssp_spline(xx_plot_freq, 0.,
                                                 grid=False),
    ...
    axes_ebl.fill_between(xx_plot, y - yerr_prop, y + yerr_prop,
    ...
    axes_sfr.plot(x_sfr, sfr(x_sfr, m2), '-',
                  color=colors[nkey], lw=2)
